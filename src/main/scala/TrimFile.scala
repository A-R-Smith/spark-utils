...
class TrimFile {
  ...
  def processFiles(dir : String): Unit = {
    val imeiDf = spark.read.format("com.databricks.spark.csv")
      ...
      .option("delimiter",",")
      .load(imeiFile)
//    spark.conf.getAll().for
    val fs = FileSystem.get(new Configuration())
    val list = fs.listStatus(new Path(dir))
    list.foreach(fn=>{
      val fileName = fn.getPath.toString
      val outfName = outDir+fileName.split("/").last
      val df = trimFile(fileName,imeiDf)
      println("Writing:"+outfName)
      df.write.parquet(outfName)
    })
  ...
  def trimFile(fileName : String, imeiDf : DataFrame): DataFrame = {
    ...
    val date = findDate(fileName)
    val smImeiDf = imeiDf.filter($"date" === date).withColumnRenamed("IMEI","DIMEI")
    val joined = inputDf.join(smImeiDf,inputDf("IMEI")===smImeiDf("DIMEI"))
    joined.drop("DIMEI","date","monthly sum")
  }

  def findDate(s : String) : String = {
    val dateReg = raw"\d{4}-\d{2}-\d{2}".r
    val it = dateReg.findAllIn(s)
    var outS = it.next();
    while (it.hasNext) {
      outS = it.next();
    }
